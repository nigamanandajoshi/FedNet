"""FedNet: Federated Learning with Compliance Audit & Solana Attestation"""
...
print("=" * 80)
print("FedNet: Federated Learning with Compliance Audit & Solana Attestation")
print("=" * 80)
...
def run_fl_simulation(rounds=3, local_epochs=2):
    ...
    # Layer 1: Initialize Audit Artifact Generator
    artifact_generator = create_artifact_generator(secret_key=admin_pk[:64])
    artifacts_dir = Path("artifacts")
    artifacts_dir.mkdir(exist_ok=True)

    print("\n" + "=" * 80)
    print(f"Configuration: {rounds} FL rounds, {local_epochs} local epochs per round")
    print("Layer 1: Audit Artifact Generation ENABLED")
    print("=" * 80)
    ...
    for round_num in range(rounds):
        ...
        if governance.can_aggregate(participating_addresses):
            ...
            print(f"\n>>> Generating Compliance Audit Artifact...")
            gradient_dict = {k: v.cpu().numpy() for k, v in new_global_weights.items()}

            audit_artifact = artifact_generator.generate_artifact(
                round_id=round_num + 1,
                participants=participating_addresses,
            ...
            artifact_generator.save_artifact(audit_artifact, str(artifact_path))

            print(f"  ✓ Audit artifact generated and signed")
            print(f"  ✓ Participants (hashed): {len(audit_artifact.participants)}")
    ...
    # List generated audit artifacts
    print(f"\n✓ Audit Artifacts Generated:")
    for artifact_file in sorted(artifacts_dir.glob("*.json")):
        loaded = artifact_generator.load_artifact(str(artifact_file))
        verified = "✓ VERIFIED" if artifact_generator.verify_artifact(loaded) else "✗ FAILED"
        print(f"    - {artifact_file.name}: Round {loaded.round_id}, {len(loaded.participants)} participants {verified}")
    ...
    print(f"\n{'='*80}")
    print("FEDERATED LEARNING WITH AUDIT COMPLETE!")
    print(f"{'='*80}")
    print(f"✓ Layer 1: Audit artifact generation — ENABLED")
    print(f"✓ Generated {len(list(artifacts_dir.glob('*.json')))} signed compliance artifacts")
    print(f"✓ Trained for {rounds} rounds across {len(hospitals)} hospitals")
    print(f"✓ Final model saved + cryptographically verified")
    print(f"✓ Artifacts stored in: {artifacts_dir.absolute()}")
    print("\nNext: Deploy artifacts to Solana for tamper-proof attestation (Layer 2)!")
